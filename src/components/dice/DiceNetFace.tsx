...
export const DiceNetFace: React.FC<DiceNetFaceProps> = ({ face, index, geometry, scale, relation, neighbors,
  ...
  return <button type="button" className={`dice-net-face is-${relation}`}
    aria-label={`第 ${index + 1} 面，${creature.name}，基礎攻擊 ${effective.baseValue}。${creature.description} 相鄰面：${neighbors.map((n) => n + 1).join('、')}。${onApply ? '點擊套用貼紙。' : ''}`}
    ...
    onBlur={() => onFocus(null)}
    onClick={onApply}
    style={{ left: bounds.x * scale, top: bounds.y * scale,
