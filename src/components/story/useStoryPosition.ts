...
export function useStoryPosition(ref: RefObject<HTMLDivElement | null>, groupIndex: number, control: boolean, passive: boolean) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const viewport = { x: vv?.offsetLeft ?? 0, y: vv?.offsetTop ?? 0,
      ...
      const position = getStoryPosition(viewport, element.getBoundingClientRect(), random.current, anchor, preferred.current);
  ...
  }, [ref, groupIndex, control, passive]);
  return layout;
}
